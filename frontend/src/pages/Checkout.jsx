...
const Checkout = () => {
  ...
  const handleProcessOrder = async (e) => {
    ...
    try {
      ...
      setShowSuccessModal(true); // Activar el modal en lugar de alert
      clearCart(); 
  ...
  return (
    <div className="min-h-screen bg-surface py-12 px-4 relative">
      <div className="max-w-7xl mx-auto">
        
        {/* Header Race Ready */}
        <div className="mb-12 border-b-2 border-ui-border pb-6">
          <h1 className="text-5xl font-black text-text-primary italic uppercase tracking-tighter">
            Finalizar <span className="text-primary">Compra .</span>
          </h1>
          <p className="text-text-secondary font-bold text-sm mt-2 uppercase tracking-widest flex items-center gap-2">
            <ShieldCheck size={16} className="text-primary" /> Checkout de alto rendimiento
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white border-2 border-ui-border p-8 relative">
               <h2 className="text-2xl font-black text-text-primary uppercase italic mb-6 flex items-center gap-3">
                 <Truck className="text-primary" /> Punto de Entrega
               </h2>
               {addresses.length > 0 && (
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                   {addresses.map(addr => (
                     <div 
                       ...
                       className={`relative p-4 border-2 cursor-pointer transform -skew-x-2 transition-all ${
                         selectedAddressId === addr.id_key ? 'border-primary bg-primary/5' : 'border-ui-border hover:border-text-primary'
                       }`}
                     >
                   ...
                   <button onClick={() => setSelectedAddressId('new')} className={`p-4 border-2 border-dashed transform -skew-x-2 flex flex-col items-center justify-center transition-colors ${selectedAddressId === 'new' ? 'border-primary text-primary bg-primary/5' : 'border-ui-border text-text-muted hover:border-text-primary'}`}>
            ...
            <section className="bg-white border-2 border-ui-border p-8">
               <h2 className="text-2xl font-black text-text-primary uppercase italic mb-6 flex items-center gap-3">
                 <CreditCard className="text-primary" /> Sistema de Pago
               </h2>
               <div className="bg-surface p-4 border-l-4 border-primary mb-6 text-[10px] font-black uppercase tracking-widest">Tarjeta de Débito Seleccionada</div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white border-2 border-text-primary p-6 sticky top-24">
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="flex justify-between text-[10px] font-black text-text-secondary uppercase">
                  <span>Logística</span>
                  {shippingCost === 0 ? <span className="text-primary font-black">GRATIS</span> : <span className="font-mono">${shippingCost.toFixed(2)}</span>}
                </div>
              </div>
              ...
              <button onClick={handleProcessOrder} disabled={loading} className="w-full bg-primary hover:bg-primary-hover text-white py-5 font-black uppercase tracking-widest text-sm transform -skew-x-12 transition-all active:scale-95 disabled:bg-ui-border flex items-center justify-center gap-3 group">
                <span className="transform skew-x-12 flex items-center gap-3">
                  {loading ? 'Procesando...' : 'Confirmar y Pagar'} <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
                </span>
              </button>
            </div>
          </div>
      ...
      {/* SUCCESS MODAL (REEMPLAZA AL ALERT) */}
      <AnimatePresence>
        {showSuccessModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            <motion.div 
              ...
              animate={{ opacity: 1 }} 
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-text-primary/95 backdrop-blur-sm"
            />
            <motion.div 
              initial={{ opacity: 0, scale: 0.9, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 20 }}
              className="relative bg-white border-t-8 border-primary p-10 max-w-md w-full text-center shadow-2xl"
            >
              ...
              <h2 className="text-4xl font-black italic uppercase tracking-tighter text-text-primary mb-2">
                ¡ORDEN COMPLETADA!
              </h2>
              <p className="text-text-secondary font-medium mb-8">
                Tu equipo está listo para la pista. Hemos enviado los detalles a tu perfil.
              </p>
              
              <div className="space-y-4">
                <button 
                  onClick={() => navigate('/profile')}
                  className="w-full bg-text-primary text-white py-4 font-black uppercase tracking-widest text-sm transform -skew-x-12 hover:bg-black transition-all"
                >
                  <span className="transform skew-x-12 inline-block">Ver Mis Pedidos</span>
                </button>
                <button 
                  onClick={() => navigate('/')}
                  className="w-full bg-transparent text-text-secondary py-2 font-bold uppercase tracking-widest text-[10px] hover:text-text-primary transition-all"
                >
                  Volver a la Tienda
                </button>
              </div>
